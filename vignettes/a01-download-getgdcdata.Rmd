...
tcga_sample_sheet <- read_tsv(file.path(raw_data_dir, "tcga_sample_sheet.tsv"))
...
cat(unlist(tcga_sample_sheet$file_id),
    file = file.path(raw_data_dir, "tcga_filename_list.txt"),
    sep = "\n")
```
...
```{r read_faildwnlds}
failed_downloads <- readLines("failed_downloads.txt")
...
```{r save_faildwnlds}
tcga_sample_sheet %<>% 
    mutate(downloaed = !(file_id %in% failed_downloads))
save(tcga_sample_sheet,
     file= file.path(data_dir, "tidy_tcga_sample_sheet.RData"))
```
...
tcga_sample_sheet %>%
    filter(!downloaded) %>%
    group_by(project_id) %>%
    summarise(num_failed_downloaded = n_distinct(file_id)) %>%
    kable() %>%
